...
import { ConfiguracionAlternativaSchema, type ConfiguracionAlternativa } from 'shared/schemas/horarios';
...
export function DrawerConfig({ tiempoComidaId, onClose }: DrawerConfigProps) {
  const {
    datosBorrador,
  ...
  const onSubmit = (data: FormData) => {
    ...
    const newConfig: ConfiguracionAlternativa = {
      ...data,
      tiempoComidaId,
      estaActivo: editingId ? datosBorrador.configuracionesAlternativas[editingId]?.estaActivo ?? true : true,
    };
  ...
  const handleEdit = (id: string, config: ConfiguracionAlternativa) => {
    ...
    setValue('definicionAlternativaId', config.definicionAlternativaId);
    setValue('horarioSolicitudComidaId', config.horarioSolicitudComidaId);
    setValue('ventanaServicio.horaInicio', config.ventanaServicio.horaInicio);
    setValue('ventanaServicio.horaFin', config.ventanaServicio.horaFin);
    setValue('requiereAprobacion', config.requiereAprobacion);
    setIsFormOpen(true);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end">
      ...
      <div className="relative w-full bg-white rounded-t-2xl max-h-[90vh] overflow-y-auto p-4 transition-transform duration-300 ease-in-out transform translate-y-0">
        ...
        {!isFormOpen && (
          <button onClick={() => { setIsFormOpen(true); setEditingId(null); reset(); }} className="w-full py-2 px-4 bg-blue-600 text-white rounded-lg font-semibold">
            + Añadir Alternativa
          </button>
        )}
