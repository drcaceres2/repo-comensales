...
import { dayKeyToDateFnsNumber, dayOfWeekKeyToDate, formatoIsoCompleto1 } from '@/lib/fechasResidencia';
...
const estaDentroFechasSinHora = (fecha: Date | string, fechaInicio?: string, fechaFin?: string): boolean => {
  if (!fecha || !fechaInicio || !fechaFin) {
    console.log('No se obtuvieron una fechas (estaDentroFechas)')
    return false;
  }
  if(fecha typeOf string) {
    dateToCompare = 
  }
  const startDate = formatoIsoCompleto1({fecha: fechaInicio, zonaHoraria: residencia.zonaHoraria });  
  const endDate = formatoIsoCompleto1({fecha: fechaFin, zonaHoraria: residencia.zonaHoraria });
