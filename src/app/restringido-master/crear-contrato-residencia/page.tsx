...
import { ContratoResidencia, ContratoResidenciaId, Cliente, ClienteId, ContactoResponsable, RecordatorioVencimiento, Suscripcion, EstadoContrato, ClienteProbando } from '@/../../shared/models/contratos'; // Adjust path as needed
...
import TimezoneSelector from '@/components/ui/TimezoneSelector'; // Adjust path as needed
...
export default function CrearContratoResidenciaPage() {
  ...
  const [selectedResidencias, setSelectedResidencias] = useState<ResidenciaId[]>([]);
  const [allResidencias, setAllResidencias] = useState<{ id: ResidenciaId; nombre: string }[]>([]);
  ...
  const [fechaInicioTimezone, setFechaInicioTimezone] = useState<string>('UTC');
  ...
  const [fechaFinTimezone, setFechaFinTimezone] = useState<string>('UTC');
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    let finalFechaInicio: campoFechaConZonaHoraria | undefined = undefined;
    if (fechaInicioDate && fechaInicioTime && fechaInicioTimezone) {
        const [hours, minutes] = fechaInicioTime.split(':').map(Number);
        const dateWithTime = new Date(fechaInicioDate.getFullYear(), fechaInicioDate.getMonth(), fechaInicioDate.getDate(), hours, minutes);
        finalFechaInicio = {
            fecha: formatInTimeZone(dateWithTime, fechaInicioTimezone, "yyyy-MM-dd HH:mm"),
            zonaHoraria: fechaInicioTimezone,
        };
    } else {
        toast({ title: 'Error de Validación', description: 'Fecha de inicio incompleta.', variant: 'destructive' });
        setIsLoading(false);
        return;
    }
    
    // --- Construct campoFechaConZonaHoraria for fechaFin ---
    let finalFechaFin: campoFechaConZonaHoraria | undefined | null = null;
    if (!esIndefinido) {
        if (fechaFinDate && fechaFinTime && fechaFinTimezone) {
            const [hours, minutes] = fechaFinTime.split(':').map(Number);
            const dateWithTime = new Date(fechaFinDate.getFullYear(), fechaFinDate.getMonth(), fechaFinDate.getDate(), hours, minutes);
            finalFechaFin = {
                fecha: formatInTimeZone(dateWithTime, fechaFinTimezone, "yyyy-MM-dd HH:mm"),
                zonaHoraria: fechaFinTimezone,
            };
        } else {
            toast({ title: 'Error de Validación', description: 'Fecha de fin incompleta y el contrato no es indefinido.', variant: 'destructive' });
            setIsLoading(false);
            return;
        }
    }
    ...
    if (isEditing && editingContratoId) {
        ...
        const updatedContratoData: Partial<Omit<ContratoResidencia, 'id' | 'cliente' | 'fechaCreacionObjeto'>> = {
            residencias: selectedResidencias,
        ...
        try {
            ...
            await writeClientLog(
                authUser.uid as UserId, 'log_contrato',
        ...
        const newContratoData: Omit<ContratoResidencia, 'id'> = {
            cliente: finalClienteId, // CRITICAL: Use the determined ClienteId
            residencias: selectedResidencias,
            fechaInicio: finalFechaInicio,
            fechaFin: esIndefinido ? undefined : finalFechaFin,
            esIndefinido: esIndefinido,
  ...
  const handleEditContrato = async (id: ContratoResidenciaId) => { // Made async
    ...
    setSelectedResidencias(contratoToEdit.residencias);
    if (contratoToEdit.fechaInicio) {
      ...
      setFechaInicioTimezone(contratoToEdit.fechaInicio.zonaHoraria);
    }
    ...
    if (!contratoToEdit.esIndefinido && contratoToEdit.fechaFin) {
      ...
      setFechaFinTimezone(contratoToEdit.fechaFin.zonaHoraria);
    } else {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Label htmlFor="clienteDePrueba">Cliente de Prueba</Label>
        </div>
        ...
        <div>
            <Label>Residencias Asociadas (IDs)</Label>
            {/* Placeholder for multi-select or dynamic input for residencias */}
            <Input 
                value={selectedResidencias.join(', ')} 
                onChange={(e) => setSelectedResidencias(e.target.value.split(',').map(id => id.trim() as ResidenciaId).filter(id => id))}
                placeholder="ID1, ID2, ID3"
            />
            {/* In a real app, this would be a more sophisticated multi-select component */}
            <p className="text-sm text-gray-500 mt-1">
                IDs de residencias disponibles: {allResidencias.map(r => `${r.nombre} (${r.id})`).join('; ') || 'Cargando residencias...'}
            </p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
            <Input 
              type="date" 
              value={fechaInicioDate ? formatInTimeZone(fechaInicioDate, 'UTC', 'yyyy-MM-dd') : ''} 
              onChange={(e) => setFechaInicioDate(e.target.value ? toDate(e.target.value, { timeZone: 'UTC' }) : undefined)}
            />
            <Input 
              type="time" 
            ...
            />
            <TimezoneSelector
              initialTimezone={fechaInicioTimezone}
              onTimezoneChange={setFechaInicioTimezone}
            />
        ...
        {!esIndefinido && (
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              <Input 
                type="date" 
                value={fechaFinDate ? formatInTimeZone(fechaFinDate, 'UTC', 'yyyy-MM-dd') : ''} 
                onChange={(e) => setFechaFinDate(e.target.value ? toDate(e.target.value, { timeZone: 'UTC' }) : undefined)}
              />
              <Input 
                type="time" 
                value={fechaFinTime} 
              ...
              />
              <TimezoneSelector
                initialTimezone={fechaFinTimezone}
                onTimezoneChange={setFechaFinTimezone}
              />
          ...
          <ul className="space-y-4">
            {contratos.map((contrato) => (
              <li key={contrato.id} className="p-4 border rounded-md shadow-sm">
                ...
                <p>Residencias: {contrato.residencias.join(', ')}</p>
