...
* **Comentario:** Son notas que los usuarios ingresan en el sistema para que el director las observe cuando haga la solicitud formal de las comidas. Para el `director` es una bandeja de tareas con distintos estados (`pendiente`, `atendido`, `diferido`, `archivado`).
...
* **Residente:** Gestiona su propia "intención" (`SemanarioUsuario`, `Excepcion`, `Ausencia`) dentro del plazo correcto, se inscribe (o acepta invitaciones) en una `Actividad` disponible, puede invitar a otros a inscribirse en una `Actividad`. Puede crear un `Comentario` para que el director lo tome en cuenta al solicitar los comensales. Otro usuario `asistente` puede ingresar los horarios de un `residente`.
